	...
	<div class="utama">
		...
		<table cellspacing="1" align="center" border="1">
			...
			if ($bulan == $ambil_bulan) {
				...
				$sql = mysqli_query($konek, "SELECT SUM(hadir) AS hadir, SUM(sakit) AS sakit, SUM(ijin) AS izin, SUM(alpa) AS alpa, nis, nama, rombel, tgl_absen, id_rombel FROM query_absen WHERE nis = '$_GET[nis]' ");
			$no = 0;
