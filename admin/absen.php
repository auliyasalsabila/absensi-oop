...
<form method="post">
	...
	if (isset($_POST['cetak'])) {
		$r = $_POST['rombel'];
		echo "<script>document.location.href='?menu=absen&rombel=$r'</script>";
	}
	if (!empty($_GET['rombel'])) {
		$tgl_sekarang = date('Y-m-d');
		$jalan = mysqli_query($konek, "SELECT * FROM query_absen WHERE id_rombel = '$_GET[rombel]' and tgl_absen = '$tgl_sekarang' ");
		$cek = mysqli_fetch_array($jalan);
	...
	<table border="1" align="center">
		...
		<tr>
			...
			<td><input type="radio" name="keterangan<?php echo $r['nis']; ?>" value="izin"></td>
